...
#将分割结果写入文件。它逐行写出每个说话人片段的起始时间、结束时间和标签
...
if __name__ == '__main__':
    ...
    #读取x-vector分段信息和PLDA模型，并进行必要的矩阵运算以准备PLDA参数。
    ...
    #打开ark文件并读取每个录音的x-vectors，按录音名称分组处理
    ...
    for file_name, segs in recit:
        ...
        if args.init == 'AHC' or args.init.endswith('VB'):
            ...
            if args.init.endswith('VB'):
                #先用AHC得到的硬标签进行平滑，生成软标签，然后应用VB-HMM进行进一步聚类
