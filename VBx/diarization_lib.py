...
#使用共享方差的两高斯混合模型（GMM）对分数 s 进行校准
#返回值：用于分离两高斯分布的阈值和校准的线性对数比值分数数组
...
#使用预训练的PLDA模型计算输入x-vectors的NxN配对相似性评分矩阵
#返回值：配对相似性评分矩阵
...
#从Kaldi的segments文件中加载每个x-vector的时间信息
#返回值：包含录音文件名、x-vector名称和时间信息的字典
...
#合并相邻或重叠的标签段，使得具有相同标签的相邻或重叠片段合并为一个段
#返回值：合并和调整后的起始时间、结束时间和标签数组
