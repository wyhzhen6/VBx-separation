...
def Processing(sep_pipeline, args, fn, model, label_name, input_name, ark_file, seg_file):
    ...
    for segnum in range(len(labs)):
        ...
        if seg.shape[0] > 0.01*samplerate:  # process segment only if longer than 0.01s
            ...
            if args.separator == 'none':
                sep_wav = np.zeros((seg.shape[0], 2))
                sep_wav[:,0]=seg
                #sep_wav = seg.reshape(-1,1)
            elif args.separator == 'pyannote':
                sf.write(f'{args.tmp_dir}/tmp.wav', seg / 2**15, samplerate)
                try:
                    sep_wav = sep_pipeline(f'{args.tmp_dir}/tmp.wav')[1].data
                    sep_wav = sep_wav[:seg.shape[0], :]
                    sep_wav = sep_wav*2**15 
                except:
                    sep_wav = seg.reshape(-1,1)
            elif args.separator == 'nass':
                sep_wav = nass.nass_separator(f'{args.tmp_dir}/tmp.wav')[0].detach().cpu().numpy()
            ...
            for i in range(sep_wav.shape[1]):
                fea = features.fbank_htk(sep_wav[:,i], window, noverlap, fbank_mx,
                ...
                for start in range(0, slen - seg_len, seg_jump):
                    ...
                    key = f'{fn}_{i:03}-{segnum:04}-{start:08}-{(start + seg_len):08}'
                ...
                if slen - start - seg_jump >= 10:
                    ...
                    key = f'{fn}_{i:03}-{segnum:04}-{(start + seg_jump):08}-{slen:08}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--separator', type=str, default='none', help='separator')
    ...
    parser.add_argument('--tmp-dir', required=True, type=str, help='tmp directory')
    ...
    if args.separator == 'pyannote':
        from pyannote.audio import Pipeline
        sep_pipeline = Pipeline.from_pretrained(
            "pyannote/speech-separation-ami-1.0",
            use_auth_token="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",
        )
    else:
        sep_pipeline = None
            ...
            with open(args.out_ark_fn, 'wb') as ark_file:
                for fn in file_names:
                    with Timer(f'Processing file {fn}'):
                        ...
                        Processing(sep_pipeline, args, fn, model, label_name, input_name, ark_file, seg_file)
